...
const TAB_KEY_ALIASES = [
  [/\bdhamal\b/gi, ''],
  // Bindal's real Sheet abbreviates "Jumbo Container" as "J CONTAINER" specifically for the N200 tab's
  // block (e.g. "N200 J CONTAINER" vs. the Production Register's "N200 Jumbo Container"). Confirmed
  // directly against the real Sheet, not guessed. Scoped narrowly to "j container" (not a bare "j")
  // because Bindal separately uses standalone "J" to mean something else in "N 100 J XL Container" /
  // "N100J XL CONTAINER" — a distinct real item from "N 100 Jumbo Container" — so a bare j->jumbo swap
  // would have wrongly merged those two different items.
