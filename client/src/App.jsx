...
CRITICAL — you are shown ONE PAGE at a time, in isolation, and this may be one page out of several in a multi-page file. Not every page in a file like this is its own genuine spec sheet worth extracting: a page can be blank, a signature/approval-only page, a plain re-typed or OCR'd text dump of a spec instead of the actual laid-out form, or otherwise not showing the header info block (Product/Shelf Life/dates) AND the numbered S.No/TEST/SPECIFICATION table in genuinely readable form. Before extracting anything, decide whether THIS page actually shows that — clearly enough to read real values from, not a guess. If it does, extract normally. If it does NOT — for any reason — return "item_code" as an empty string and leave every other field at its default/empty/zero value. That is the correct, expected answer for a page like that, not a failure, and not something to apologize for or explain.
