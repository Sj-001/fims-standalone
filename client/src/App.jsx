...
FINAL COUNT CHECK — MANDATORY BEFORE YOU FINALIZE: count the actual number of reel line items printed on this slip (excluding "Total"/"Grand Total" rows). Your "items" array must contain EXACTLY that many entries, no more and no fewer. If your count is off, a reel was skipped somewhere in the middle — go find it and insert it in the right place; do NOT compensate by inventing an extra item or duplicating one you already have. Every item must correspond to one real, physically-printed reel line you can point to on the slip.`,
    shape: (raw) => {
...
- NEVER LET THE ITEM NAME'S OWN NUMBERS LEAK INTO THE QUANTITY: many item names contain a number that's part of the product code itself, not a quantity — "IT 500", "N 200", "T 50" are product codes, the "500"/"200"/"50" there is not a piece count. The quantity always comes from its own separate column, physically apart from the description. If the quantity column is faint or hard to read, re-examine ONLY that column's own digits — never construct a quantity by combining or confusing it with a digit that's actually part of the item name right next to it (e.g. reading "IT 500" plus a faint "1480" in the quantity column as "17500" is exactly the mistake to avoid — the item code and the quantity are two separate numbers written in two separate places, never merge them). If you are not genuinely confident in the quantity digits after that re-check, set "flag" rather than output a number assembled from the wrong place.
...
FINAL COUNT CHECK — MANDATORY BEFORE YOU FINALIZE: count the actual number of physical rows on this page that have genuine handwritten content (not a blank pre-printed SL.NO. slot — see the blank-line rule above). Your "items" array must contain EXACTLY that many entries, no more and no fewer. If your count is off, a real row was skipped somewhere in the middle — go find it and insert it in the right place; do NOT compensate by inventing an extra item at the end or duplicating one you already have. Every item must correspond to one real, physically-written row you can point to on the page — this matters especially when the SAME page is being re-uploaded because new rows were added below earlier ones: every one of those earlier rows still needs to come through correctly too, not just the new ones at the bottom.
...
{{ABBREVIATIONS}}
FINAL COUNT CHECK — MANDATORY BEFORE YOU FINALIZE, on the Original page only: count the actual number of distinct line items listed under "Description of Goods" on this invoice. Your "items" array must contain EXACTLY that many entries, no more and no fewer. If your count is off, an item was skipped — go find it and insert it in the right place; do NOT compensate by inventing an extra item or duplicating one you already have.
Return ONLY one JSON object: {"invoice_no":"","date":"","party":"","buyer_order_no":"","items":[{"description":"","quantity":number,"rate":number,"amount":number}]}`,
  ...
  },
  {
    ...
    register: 'daburSpecs',
    systemPrompt: `You read a printed "Dabur India Limited" PM/packaging specification sheet for a corrugated box. Different print runs of this document use different layouts — adapt to whichever one this specific page actually uses, using the row LABELS (Product/Item, Item Code, Number of Plies, Length, Width, Height, Paper Combination, Partition Size, Central Plate, etc.) as your anchor, not one fixed column structure. Two layouts seen in practice:
- LAYOUT A: header has a single "Product" line combining the code and name (e.g. "1223069359 CFB Ghrit Common- New specs"); the spec table has columns S.No / TEST / SPECIFICATION / STANDARD TEST PROCEDURE, with a row's full tolerance range written as one combined text value (e.g. "338.00 - 344.00mm").
- LAYOUT B: header has SEPARATE "Item:" and "Item Code:" fields instead of one combined "Product" line; the spec table has columns like TEST ID / TEST NAME / SPECIFICATION / LOWER VALUE / TARGET / UPPER VALUE / UOM / STP REFERENCE, with a row's range split into separate LOWER VALUE and UPPER VALUE numeric columns instead of one combined text value (ignore the TARGET column — it's neither the min nor the max).
Extract ONLY the specific fields listed below — ignore Shelf Life, Creation/Revision/Valid dates, Visual Check/Description defect-criteria text, Total GSM (Box), Bursting factor, Flutes type, Flute Direction, Thickness (board), Compression Strength, Moisture Content, Packing configuration, and any Instructions-to-Plant/Vendor/Storage rows — none of those are used here.
CRITICAL — you are shown ONE PAGE at a time, in isolation, and this may be one page out of several in a multi-page file. Default to extracting normally — most pages in a batch like this ARE genuine spec pages, just possibly in the less-common layout above, or with a slightly different header block than either example. Only return an empty "item_code" (leaving every other field at its default/empty/zero value) when this page is CLEARLY not a spec sheet at all — genuinely blank, a bare signature/approval stamp with no spec content, or unrelated to Dabur PM specifications entirely. Do not skip a page just because it doesn't match Layout A or Layout B exactly, or because a column is labeled slightly differently than expected — read the actual row labels and values in front of you.
Return ONLY one JSON object:
{"item_code":"the item code — the leading numeric code from a combined Product line (Layout A), or the separate Item Code field (Layout B)","item_name":"the item name — the rest of the Product line after the code (Layout A), or the separate Item field (Layout B)","no_of_ply":number (from Number of Plies),"box_length_min":number,"box_length_max":number,"box_width_min":number,"box_width_max":number,"box_height_min":number,"box_height_max":number,"paper_comb":"the Paper Combination and GSM text exactly as printed, e.g. '140(VK)/120(SK)/120(SK)/120(SK)/150(SK) ±10%'","partition_tier_count":number,"partition_longer_length_mm":number,"partition_longer_height_mm":number,"partition_longer_qty_total":number,"partition_shorter_width_mm":number,"partition_shorter_height_mm":number,"partition_shorter_qty_total":number,"paper_comb_partitions":"the Paper combination (Partition) text exactly as printed","no_of_plate":number,"paper_comb_plate":"the Paper combination (Plate) text exactly as printed","plate_length_mm":number,"plate_width_mm":number}
FIELD NOTES:
- box_length_min/max, box_width_min/max, box_height_min/max: from the Length/Width/Height rows. Layout A: one combined range like "338.00 - 344.00mm" -> min=338, max=344 (strip "mm"). Layout B: separate LOWER VALUE / UPPER VALUE columns on that row -> min=LOWER VALUE, max=UPPER VALUE. Keep the numbers exactly as printed, do not average them yourself.
...
- Never do any arithmetic yourself — no averaging, no unit conversion, no multiplying tier counts. Extract every number exactly as printed; all calculation happens after this, outside your response.`,
    shape: (raw) => {
      // A page that isn't a genuine spec page (blank, a signature page, an OCR text dump, a
      // continuation page — see the prompt above) comes back with an empty item_code on purpose. That
      // must produce ZERO rows here, not one row full of blanks/zeros needing to be noticed and
      // manually discarded — confirmed directly: a 15-page upload where several pages weren't real
      // spec pages was showing a garbage all-zero row to review for each one.
      if (!raw || !String(raw.item_code || '').trim()) return [];
  ...
  },
  {
    ...
    register: 'daburPO',
    systemPrompt: `You read a printed Dabur purchase order (may be titled "Draft Purchase Order").
FINAL COUNT CHECK — MANDATORY BEFORE YOU FINALIZE: count the actual number of distinct material line items listed on this PO. Your "items" array must contain EXACTLY that many entries, no more and no fewer. If your count is off, an item was skipped — go find it and insert it in the right place; do NOT compensate by inventing an extra item or duplicating one you already have.
Return ONLY one JSON object:
...
FINAL COUNT CHECK — MANDATORY BEFORE YOU FINALIZE, on the Original page only: count the actual number of distinct line items listed on this invoice. Your "items" array must contain EXACTLY that many entries, no more and no fewer. If your count is off, an item was skipped — go find it and insert it in the right place; do NOT compensate by inventing an extra item or duplicating one you already have.
Return ONLY one JSON object:
...
function FIMSApp() {
  ...
  // datalist suggestions in Pending Review and Customer Sheets.
  ...
  // Editing a flagged row's cell IS the review the flag was asking for — confirmed directly as a real
  // bug: correcting a flagged value left the row still marked flagged, which kept excluding that whole
  // page from "Confirm all" (see confirmAllPages/cleanDoneCount) even after the person had already
  // fixed it, with no way to bulk-confirm it short of re-uploading. Clears flagged/flagReason on
  // WHICHEVER row is being edited, regardless of which field — the flag isn't tied to one specific
  // column (flagReason is a free-text note, not a per-column marker), so any edit to that row is
  // treated as the person having looked at and addressed it.
  const updateReviewCell = (pageIdx, rowId, field, value) => {
    setFileResults(prev => prev.map((page, i) => i !== pageIdx ? page : { ...page, rows: page.rows.map(r => r.id === rowId ? { ...r, [field]: value, flagged: false, flagReason: '' } : r) }));
  };
  ...
  // Used by addRows for the dedup check — normalizes JUST the free-text `description` field before
  // hashing, so a re-extraction of the same physical row that comes back worded slightly differently
  // ("N200 J Container" vs "N200 Jumbo Container" — confirmed directly: same date, same 310 pieces,
  // only the abbreviation differs) still hashes to the same key and gets caught as a duplicate instead
  // of silently becoming a second real row. Reuses the SAME abbreviation-expansion already taught for
  // extraction/matching, rather than a separate normalization — teaching "J" = "Jumbo" once, in
  // Settings, fixes both extraction consistency AND this dedup check together. Deliberately goes no
  // further than that (no pack-count stripping, no unit-word removal like normalizeVariant does for
  // ledger grouping) — rowDedupKey already compares every other field exactly, and stripping more here
